...
2025 年 11 月 4 日（v2.0.1-4.0.5）
------------------------------------

工单系统功能优化
~~~~~~~~~~~~~~~~~~~~~~~

* **工单流程记录实时更新**：修复了工单状态变更时流程记录未能实时显示的问题，确保每次状态变更都会生成对应的流程记录
* **工单更新逻辑优化**：使用 ``TicketFlowService.add_flow_step()`` 方法替代直接创建流程记录对象，确保流程记录生成的一致性和完整性
* **研发部门权限扩展**：研发部门成员可以编辑状态为"未完成-研发原因"的工单，实现跨部门协作
* **工单删除功能**：新增批量删除工单功能，可保留指定ID的工单，支持删除相关流程记录
* **流程记录测试**：添加了完整的流程记录实时更新测试脚本，验证状态变更流程的正确性
