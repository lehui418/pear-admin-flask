...
def _build_ticket_query_filters(request_args: dict) -> ModelFilter:
    ...
    mf.vague('product_type_level1', str_escape(request_args.get("product_type_level1", "")))
    mf.vague('product_type_level2', str_escape(request_args.get("product_type_level2", "")))
    mf.vague('problem_classification_main', str_escape(request_args.get("problem_classification_main", "")))
    mf.vague('customer_agent_name', str_escape(request_args.get("customer_agent_name", "")))
...
def _process_ticket_data(tickets: list) -> list:
    ...
    for ticket_dict in tickets:
        ...
        ticket_dict['completion_time'] = _format_datetime(ticket_dict.get('completion_time'))
...
def _combine_image_references(image_references_str_description: str, 
                           image_references_str_relatedinfo: str, 
                           image_references_str_solution: str) -> tuple:
    """
    合并多个图片引用字符串
    ...
    if image_references_str_description: 
        combined_image_references_str += image_references_str_description + "\n@@IMAGE_SEPARATOR@@\n"
    if image_references_str_relatedinfo: 
        combined_image_references_str += image_references_str_relatedinfo + "\n@@IMAGE_SEPARATOR@@\n"
    if image_references_str_solution: 
        combined_image_references_str += image_references_str_solution + "\n@@IMAGE_SEPARATOR@@\n"
    combined_image_references_str = combined_image_references_str.strip()

    all_photo_ids = []
    if image_references_str_description:
        all_photo_ids.extend(json.loads(_extract_photo_ids(image_references_str_description)))
    if image_references_str_relatedinfo:
        all_photo_ids.extend(json.loads(_extract_photo_ids(image_references_str_relatedinfo)))
    if image_references_str_solution:
        all_photo_ids.extend(json.loads(_extract_photo_ids(image_references_str_solution)))
...
def _create_ticket_flow(ticket_id: int, flow_type: str, flow_mode: str, 
                       ticket_status: str, user_dept) -> TicketFlow:
    """
    创建工单流程记录
...
def main():
    ...
    if current_user.dept_id:
        ...
        if user_dept:
            ...
            if '质量' in user_dept.dept_name or '质检' in user_dept.dept_name or '品控' in user_dept.dept_name:
                is_quality_dept_member = True
    
    return render_template('system/ticket/main.html', is_rd_dept_member=is_rd_dept_member, is_quality_dept_member=is_quality_dept_member)
...
def table_data():
    query = Ticket.query.with_entities(
        ...
        Ticket.status, Ticket.assignee_name, Ticket.photo_ids, 
        Ticket.image_references_str, Ticket.impact_scope, Ticket.relatedinfo,
        Ticket.solution, Ticket.security_level, Ticket.threat_type,
        Ticket.attack_source, Ticket.attack_target, Ticket.vulnerability_name,
        Ticket.cvss_score, Ticket.ioc_indicators, Ticket.containment_measures,
        Ticket.eradication_measures, Ticket.recovery_measures, Ticket.lessons_learned,
        Ticket.compliance_requirements, Ticket.is_gdpr_compliant, Ticket.is_ccpa_compliant,
        Ticket.other_compliance, Ticket.service_method, Ticket.appointment_time,
        Ticket.engineer_id, Ticket.product_type_level1, Ticket.product_type_level2,
        Ticket.version_number, Ticket.serial_number, Ticket.is_out_of_warranty,
        Ticket.order_time, Ticket.completion_time, Ticket.problem_classification_main,
        Ticket.problem_classification_sub, Ticket.problem_classification_tags,
        Ticket.problem_tags, Ticket.create_time, Ticket.update_time, Ticket.customer_agent_name,
    )
...
def save():
    ...
    if current_user.is_authenticated:
        current_app.logger.info(f"当前用户已认证: {current_user.username}, ID: {current_user.id}")
    else:
    ...
    try:
        ...
        missing_fields = [field for field in required_fields if field not in req_json or not req_json[field]]
        if missing_fields:
        ...
        if not service_method:
            service_method = "远程"
            current_app.logger.warning(f"工单未提供service_method，使用默认值: {service_method}")
            
        current_app.logger.info(f"工单标题: {req_json.get('title')}, 工单状态: {req_json.get('status', '创建/提交')}")
        ...
        flow_mode = TicketFlowService.determine_flow_mode(service_method, None, ticket_status)
        ...
        if business_recovered is None or business_recovered == '':
            ...
            business_recovered = business_recovered == True or business_recovered == 'on' or business_recovered == 'true'
        current_app.logger.info(f"业务恢复状态: {business_recovered}")
        
        # 确保flow_mode不为空
        if not flow_mode or flow_mode == '未知模式':
            flow_mode = '创建/提交工单'  # 设置默认的流程模式
            current_app.logger.warning(f"Setting default flow_mode: {flow_mode} for service_method: {service_method}")
        ...
        new_ticket = Ticket(
            ...
            assignee_name=str_escape(req_json.get('assignee_name')), # 从表单获取assignee_name
            ...
            # 保修和时间字段
            is_out_of_warranty=req_json.get('is_out_of_warranty') == 'on' or req_json.get('is_out_of_warranty') == True,
            ...
            # 问题分类字段
            problem_classification_main=str_escape(req_json.get('problem_classification_main')),
            problem_classification_sub=str_escape(req_json.get('problem_classification_sub')),
            problem_classification_tags=str_escape(req_json.get('problem_tags')),  # 使用正确的字段名
            problem_tags=str_escape(req_json.get('problem_tags')),  # 保留旧字段名以确保兼容性
            user_id=current_user.id if current_user.is_authenticated else None  # 记录创建者ID
        )
        try:
            # 获取当前用户的部门信息
            user_dept = None
            if current_user.is_authenticated and hasattr(current_user, 'dept_id') and current_user.dept_id:
                user_dept = Dept.query.get(current_user.dept_id)
            
            # 先保存工单到数据库，确保获取到ID
            db.session.add(new_ticket)
            db.session.flush()  # 获取ID但不提交整个事务
            current_app.logger.info(f"工单保存成功: ID={new_ticket.id}, Title={new_ticket.title}")
            
            # 创建初始流程记录
            current_app.logger.info(f"创建初始流程记录，工单ID: {new_ticket.id}, 流程类型: {flow_type}, 流程模式: {flow_mode}")
            ...
            current_app.logger.info(f"Ticket created successfully: ID={new_ticket.id}, Title={new_ticket.title}")
        ...
        try:
            ...
            if ticket.create_time and ticket.status not in ['已关闭', '已解决']:
                try:
                    ...
                    current_app.logger.error(f"Error calculating overdue status for ticket {ticket.id} in view_view: {e}")
        except Exception as e:
            current_app.logger.error(f"Error calculating overdue status for ticket {ticket.id} in view_view: {e}")
...
def edit_view(ticket_id):
    ticket = db.session.get(Ticket, ticket_id) # 使用 db.session.get 替代 get_one_by_id 如果适用
    if not ticket:
    ...
    if ticket.create_time and ticket.status not in ['已关闭', '已解决']:
        try:
            ...
            current_app.logger.error(f"Error calculating overdue status for ticket {ticket.id} in edit_view: {e}")
    ...
    if current_user.username == 'admin':
        ...
        pass  # 允许继续执行
    # 用户可以编辑自己创建的工单
    elif hasattr(ticket, 'user_id') and ticket.user_id and ticket.user_id == current_user.id:
        pass  # 允许继续执行
    # 用户可以编辑自己负责的工单
    elif hasattr(ticket, 'assignee_name') and ticket.assignee_name and ticket.assignee_name == current_user.username:
        pass  # 允许继续执行
    # 检查前端传递的是否为负责人标志
    elif is_assignee:
        # 前端已确认用户是负责人，允许编辑
        current_app.logger.info(f"User {current_user.username} viewing edit page for ticket ID {ticket_id} as assignee")
        pass  # 允许继续执行
    # 研发部门成员可以编辑状态为"未完成-研发原因"的工单
    elif is_rd_dept_member and ticket.status == '未完成-研发原因':
        current_app.logger.info(f"R&D department member {current_user.username} viewing edit page for ticket ID {ticket_id} with status '未完成-研发原因'")
        pass  # 允许继续执行
    # 研发部门成员可以编辑状态为"暂时规避"且问题分类为特定类型的工单
    elif is_rd_dept_member and ticket.status == '暂时规避' and ticket.problem_classification_main:
        # 检查问题分类是否包含研发相关的关键词
        ...
        is_rd_related = any(keyword in problem_classification_lower for keyword in rd_keywords)
        
        if is_rd_related:
            current_app.logger.info(f"R&D department member {current_user.username} viewing edit page for ticket ID {ticket_id} with status '暂时规避' and R&D related problem classification: {problem_classification}")
            pass  # 允许继续执行
        else:
            current_app.logger.warning(f"R&D department member {current_user.username} attempted to edit ticket ID {ticket_id} with status '暂时规避' but problem classification is not R&D related: {problem_classification}")
            try:
                return render_template('errors/403.html'), 403
            except:
                return "<h1>403 Forbidden</h1><p>您没有权限编辑此工单（暂时规避状态的工单需要问题分类为研发相关类型）</p>", 403
    # 质量部权限：检查是否在业务恢复阶段且状态为"未完成-生产原因"，或在彻底修复阶段且问题分类符合质量部责任
    elif is_quality_dept_member and (
        (ticket.status == '未完成-生产原因') or
        (ticket.problem_classification_main and (
            '软件bug-需寄回升级包' in ticket.problem_classification_main or
            '硬件' in ticket.problem_classification_main
        ))
    ):
        current_app.logger.info(f"Quality department member {current_user.username} viewing edit page for ticket ID {ticket_id}")
        pass  # 允许继续执行
    else:
        current_app.logger.warning(f"User {current_user.username} attempted to edit ticket ID {ticket_id} without permission")
    ...
    if ticket:
        ...
        date_fields = ['appointment_time', 'order_time', 'completion_time', 'create_time', 'update_time']
        for field in date_fields:
            if field in ticket_data and ticket_data[field] and isinstance(ticket_data[field], str):
                if 'T' in ticket_data[field]:
                    ticket_data[field] = ticket_data[field].replace('T', ' ').split('.')[0]  # 去除可能的毫秒部分

    return render_template('system/ticket/edit.html', ticket=ticket, ticket_data_json=ticket_data, users=users)
...
def load_add_data_to_edit():
    ...
    for field in date_fields:
        if field in add_form_data and add_form_data[field] and isinstance(add_form_data[field], str):
            if 'T' in add_form_data[field]:
                add_form_data[field] = add_form_data[field].replace('T', ' ').split('.')[0]  # 去除可能的毫秒部分
    
    # 创建一个新的工单对象，但不保存到数据库
    ticket = None
    
    # 将表单数据传递给edit.html模板
    return render_template('system/ticket/edit.html', ticket=ticket, add_form_data=add_form_data)


# 更新工单
@bp.post('/update') # 通常使用 PUT，但前端 AJAX 使用 POST
@authorize("system:ticket:edit")
@operation_log(lambda: f'更新工单 -> ID: {g.ticket_id}, 标题: {g.ticket_title}\n变更内容: {g.change_info}')
def update():
    """更新工单"""
    ...
    elif (ticket.user_id and ticket.user_id == current_user.id) or (ticket.assignee_name and ticket.assignee_name == current_user.username):
    ...
    # 研发部门成员可以编辑状态为"未完成-研发原因"的工单
        ...
        if not is_rd_dept_member:
            return fail_api(msg="权限不足：只有研发部门成员可以编辑状态为'未完成-研发原因'的工单")
    # 研发部门成员可以编辑状态为"暂时规避"且问题分类为特定类型的工单
        ...
        if not is_rd_dept_member:
            return fail_api(msg="权限不足：只有研发部门成员可以编辑状态为'暂时规避'的工单")
        ...
        if not is_rd_related:
            return fail_api(msg="权限不足：暂时规避状态的工单需要问题分类为研发相关类型（软件bug-新bug需研发提供升级包、bug开发中、死机问题）")
    # 质量部权限：检查是否在业务恢复阶段且状态为"未完成-生产原因"，或在彻底修复阶段且问题分类符合质量部责任
    elif (
        (ticket.status == '未完成-生产原因') or
        (ticket.problem_classification_main and (
        ...
        if current_user.dept_id:
            from applications.models import Dept
            dept = Dept.query.get(current_user.dept_id)
            if dept and ('质量' in dept.dept_name or '质检' in dept.dept_name or '品控' in dept.dept_name):
                is_quality_dept_member = True
    ...
    for key, new_value in req_json.items():
        # 跳过id和无需比较的字段
        if key in ['id', 'image_references_str_description', 'image_references_str_relatedinfo', 'image_references_str_solution']:
            continue
        ...
        if str(old_value) != str(new_value):
            label = field_to_label.get(key, key) # 获取中文标签，如果不存在则使用原字段名
    ...
    def extract_photo_ids(image_references_str):
        ...
        try:
            ...
            current_app.logger.error(f"Error processing image_references_str for update: {e}")
            return []

    # 处理图片信息
    image_references_str_description = req_json.get('image_references_str_description', ticket.image_references_str_description if hasattr(ticket, 'image_references_str_description') else None)
    image_references_str_relatedinfo = req_json.get('image_references_str_relatedinfo', ticket.image_references_str_relatedinfo if hasattr(ticket, 'image_references_str_relatedinfo') else None)
    image_references_str_solution = req_json.get('image_references_str_solution', ticket.image_references_str_solution if hasattr(ticket, 'image_references_str_solution') else None)
    ...
    if image_references_str_description: combined_image_references_str_update += image_references_str_description + "\n@@IMAGE_SEPARATOR@@\n"
    if image_references_str_relatedinfo: combined_image_references_str_update += image_references_str_relatedinfo + "\n@@IMAGE_SEPARATOR@@\n"
    if image_references_str_solution: combined_image_references_str_update += image_references_str_solution + "\n@@IMAGE_SEPARATOR@@\n"
    ...
    try:
        ...
        req_json.get('customer_agent_name', 
        ticket.customer_agent_name if hasattr(ticket, 'customer_agent_name') else '')
        )
        ...
        current_app.logger.info(f"更新工单 - 状态: {old_status} -> {new_status}, 业务恢复: {business_recovered}")

        ticket.assignee_name = str_escape(req_json.get('assignee_name', ticket.assignee_name))
        
        # 更新服务方式相关字段
        ticket.service_method = str_escape(req_json.get('service_method', ticket.service_method if hasattr(ticket, 'service_method') else None))
        
        # 如果服务方式发生变化，更新流程类型和模式
        if ticket.service_method != req_json.get('service_method'):
            ticket.flow_type = TicketFlowService.determine_flow_type(ticket.service_method)
            ticket.flow_mode = TicketFlowService.determine_flow_mode(ticket.service_method, None, new_status)
        
        raw_appointment_time = req_json.get('appointment_time')
        ticket.appointment_time = datetime.strptime(raw_appointment_time, '%Y-%m-%d %H:%M:%S') if raw_appointment_time else (ticket.appointment_time if hasattr(ticket, 'appointment_time') else None)
        ticket.engineer_id = str_escape(req_json.get('engineer_id', ticket.engineer_id if hasattr(ticket, 'engineer_id') else None))
        # 更新产品信息字段
        ticket.product_type_level1 = str_escape(req_json.get('product_type_level1', ticket.product_type_level1 if hasattr(ticket, 'product_type_level1') else None))
        ticket.product_type_level2 = str_escape(req_json.get('product_type_level2', ticket.product_type_level2 if hasattr(ticket, 'product_type_level2') else None))
        ticket.version_number = str_escape(req_json.get('version_number', ticket.version_number if hasattr(ticket, 'version_number') else None))
        ticket.serial_number = str_escape(req_json.get('serial_number', ticket.serial_number if hasattr(ticket, 'serial_number') else None))
        ticket.product_name = str_escape(req_json.get('product_name', ticket.product_name if hasattr(ticket, 'product_name') else None))

        # 更新保修和时间字段
        is_out_of_warranty_val = req_json.get('is_out_of_warranty')
        ticket.is_out_of_warranty = is_out_of_warranty_val == 'on' or is_out_of_warranty_val == True or is_out_of_warranty_val == 'true'

        raw_order_time = req_json.get('order_time')
        ticket.order_time = datetime.strptime(raw_order_time, '%Y-%m-%d %H:%M:%S') if raw_order_time else (ticket.order_time if hasattr(ticket, 'order_time') else None)
        ...
        # 如果状态从"已关闭"变更为其他状态，则清空完成时间
            ...
            ticket.completion_time = datetime.strptime(raw_completion_time, '%Y-%m-%d %H:%M:%S') if raw_completion_time else (ticket.completion_time if hasattr(ticket, 'completion_time') else None)
        
        # 更新问题分类字段
        ticket.problem_classification_main = str_escape(req_json.get('problem_classification_main', ticket.problem_classification_main if hasattr(ticket, 'problem_classification_main') else None))
        ticket.problem_classification_sub = str_escape(req_json.get('problem_classification_sub', ticket.problem_classification_sub if hasattr(ticket, 'problem_classification_sub') else None))
        ticket.problem_classification_tags = str_escape(req_json.get('problem_classification_tags', ticket.problem_classification_tags if hasattr(ticket, 'problem_classification_tags') else None))
        ticket.problem_tags = req_json.get('problem_tags', ticket.problem_tags if hasattr(ticket, 'problem_tags') else None)
        
        # 更新问题分析与处理过程字段
        ticket.relatedinfo = req_json.get('relatedinfo', ticket.relatedinfo if hasattr(ticket, 'relatedinfo') else None)
        ticket.solution = req_json.get('solution', ticket.solution if hasattr(ticket, 'solution') else None)
        ticket.impact_scope = req_json.get('impact_scope', ticket.impact_scope if hasattr(ticket, 'impact_scope') else None)
        ticket.security_level = req_json.get('security_level', ticket.security_level if hasattr(ticket, 'security_level') else None)
        ticket.threat_type = req_json.get('threat_type', ticket.threat_type if hasattr(ticket, 'threat_type') else None)
        ticket.attack_source = req_json.get('attack_source', ticket.attack_source if hasattr(ticket, 'attack_source') else None)
        ticket.attack_target = req_json.get('attack_target', ticket.attack_target if hasattr(ticket, 'attack_target') else None)
        ticket.vulnerability_name = str_escape(req_json.get('vulnerability_name', ticket.vulnerability_name if hasattr(ticket, 'vulnerability_name') else None))
        ticket.cvss_score = req_json.get('cvss_score', ticket.cvss_score if hasattr(ticket, 'cvss_score') else None)
        ticket.ioc_indicators = str_escape(req_json.get('ioc_indicators', ticket.ioc_indicators if hasattr(ticket, 'ioc_indicators') else None))
        ticket.containment_measures = str_escape(req_json.get('containment_measures', ticket.containment_measures if hasattr(ticket, 'containment_measures') else None))
        ticket.eradication_measures = str_escape(req_json.get('eradication_measures', ticket.eradication_measures if hasattr(ticket, 'eradication_measures') else None))
        ticket.recovery_measures = req_json.get('recovery_measures', ticket.recovery_measures if hasattr(ticket, 'recovery_measures') else None)
        ticket.lessons_learned = req_json.get('lessons_learned', ticket.lessons_learned if hasattr(ticket, 'lessons_learned') else None)
        # 更新详细记录字段
        ticket.description = req_json.get('description', ticket.description if hasattr(ticket, 'description') else None)
        ...
        # 检查问题分类是否发生变化
        old_problem_classification_main = original_data.get('problem_classification_main', '') or ''
        new_problem_classification_main = req_json.get('problem_classification_main', '') or ''
        old_problem_classification_sub = original_data.get('problem_classification_sub', '') or ''
        new_problem_classification_sub = req_json.get('problem_classification_sub', '') or ''
        
        problem_classification_changed = (
            old_problem_classification_main != new_problem_classification_main or
            old_problem_classification_sub != new_problem_classification_sub
        )
        
        if problem_classification_changed:
            current_app.logger.info(f"问题分类变化 - 主分类: '{old_problem_classification_main}' -> '{new_problem_classification_main}', 子分类: '{old_problem_classification_sub}' -> '{new_problem_classification_sub}'")
        ...
        # 添加详细的流程节点创建判断日志
        current_app.logger.info(f"流程节点创建判断 - 状态变更: {has_status_change}, 处理记录更新: {relatedinfo_updated}, 处置方案更新: {solution_updated}")
        if 'relatedinfo' in req_json:
            current_app.logger.info(f"处理记录对比 - 原值: '{old_relatedinfo_value}', 新值: '{new_relatedinfo_value}', 是否变化: {relatedinfo_updated}")
        if 'solution' in req_json:
            current_app.logger.info(f"处置方案对比 - 原值: '{old_solution_value}', 新值: '{new_solution_value}', 是否变化: {solution_updated}")
        ...
        if has_status_change:
            try:
                ...
                current_app.logger.info(f"工单SLA状态更新: ID={ticket_id}, 原状态={old_status}, 新状态={new_status}")
            except Exception as sla_error:
                current_app.logger.error(f"工单SLA状态更新失败: {str(sla_error)}")
                # SLA更新失败不影响工单状态更新
        
        security_fields_updated = []
        security_fields = ['containment_measures', 'eradication_measures', 'recovery_measures']
        ...
        # 只监控这四个关键字段的变化
        if has_status_change or relatedinfo_updated or solution_updated or problem_classification_changed:
            try:
                ...
                if not recent_flow:
                    # 获取当前用户的部门信息
                    handler_dept = None
                    if current_user and hasattr(current_user, 'dept_id') and current_user.dept_id:
                        from applications.models import Dept
                        dept = Dept.query.get(current_user.dept_id)
                    ...
                    if problem_classification_changed:
                        ...
                        if old_problem_classification_main != new_problem_classification_main:
                            classification_parts.append(f"主分类: {old_problem_classification_main or '无'} → {new_problem_classification_main or '无'}")
                        if old_problem_classification_sub != new_problem_classification_sub:
                            classification_parts.append(f"子分类: {old_problem_classification_sub or '无'} → {new_problem_classification_sub or '无'}")
                        classification_note = "问题分类变更: " + ", ".join(classification_parts)
                        if final_notes:
                    ...
                    current_app.logger.info(f"创建流程记录: 工单 {ticket_id} - {description}")
                else:
                    current_app.logger.warning(f"避免重复创建流程记录: 工单 {ticket_id} 在10秒内已有更新记录")
            except Exception as flow_error:
                current_app.logger.error(f"Error adding unified flow record for ticket {ticket_id}: {flow_error}")
    ...
    try:
        ...
        for flow in flows:
            ...
            if has_temporary_solution or is_current_temporary_solution:
                ...
                is_business_recovery_stage = (
                    flow.to_status in ['未完成-客户原因', '未完成-研发原因', '未完成-生产原因', '未完成-售后原因', '处理中']
                )
                is_complete_fix_stage = (
                    flow.to_status in ['已恢复', '已解决', '已关闭']
                )
            ...
            sla_threshold = priority_thresholds.get(ticket.priority, 24) if ticket else 24
            ...
            flow_item = {
                ...
                'create_time_str': flow.create_time.strftime('%Y-%m-%d %H:%M:%S') if flow.create_time else None,
        ...
        # 获取工单创建时间作为累计计算的起点
        ticket_create_time = ticket.create_time if ticket and ticket.create_time else (flow_data[0]['create_time'] if flow_data else now)
            ...
            if start_time and end_time and ticket_create_time:
                ...
                if cumulative_hours > sla_threshold:
                    ...
                    if in_complete_fix_stage:
                        # 彻底修复阶段
                        node_overdue_hours = current_cumulative_overdue - prev_complete_overdue
                        prev_complete_overdue = current_cumulative_overdue
                    else:
                        # 业务恢复阶段
                        node_overdue_hours = current_cumulative_overdue - prev_business_overdue
                    ...
                    flow_item['cumulative_overdue_hours'] = round(current_cumulative_overdue, 1)  # 累计超时时间
                    flow_item['cumulative_hours'] = round(cumulative_hours, 1)  # 累计时间
                    
                    # 确定责任部门
                    is_initial_node = (
                        flow_item['to_status'] in ['创建/提交'] or 
                        (flow_item['from_status'] in [None, 'None'] and flow_item['description'] == '工单创建')
                    )
                    ...
                    if is_initial_node:
                        ...
                        elif flow_item['to_status'] in ['未完成-售后原因', '处理中', '创建/提交']:
                            flow_item['responsible_department'] = "技术支持部"
                        else:
                            flow_item['responsible_department'] = "技术支持部"
                    elif flow_item['is_complete_fix_stage']:
                        problem_classification = flow_item['problem_classification_sub'] or flow_item['problem_classification_main'] or ""
                        
                        if '软件bug-新bug需研发提供升级包' in problem_classification or 'bug开发中' in problem_classification or '死机问题' in problem_classification:
                            flow_item['responsible_department'] = "研发部"
                        elif '软件bug-需寄回升级包' in problem_classification or any(keyword in problem_classification for keyword in ['硬件']):
                            flow_item['responsible_department'] = "质量部"
        ...
        if flow_data and ticket:
            ...
            # 过滤掉小于0.01小时的部门
            business_recovery_department_overdue = {k: round(v, 1) for k, v in business_recovery_department_overdue.items() if v > 0.05}
            complete_fix_department_overdue = {k: round(v, 1) for k, v in complete_fix_department_overdue.items() if v > 0.05}
            
            # 计算总超时时间
            business_recovery_overdue = round(sum(business_recovery_department_overdue.values()), 1)
            complete_fix_overdue = round(sum(complete_fix_department_overdue.values()), 1)
        ...
        return jsonify({
            ...
            'data': {
                ...
                'statistics': {
                    ...
                    'is_completed': statistics['is_completed'] if statistics else False,
                    'department_counts': statistics['department_counts'] if statistics else {},
                    'mode_counts': statistics['mode_counts'] if statistics else {}
                }
            }
        })
    except Exception as e:
        current_app.logger.error(f"Error getting ticket flow for ticket {ticket_id}: {e}")
        return jsonify({
            'code': 500,
            'msg': '获取工单流程失败',
            'data': None
...
def delete():
    ...
    # 检查前端传递的是否为负责人标志
        ...
        # 前端已确认用户是负责人，允许删除
        current_app.logger.info(f"User {current_user.username} deleting ticket ID {ticket_id} as assignee")
        pass  # 允许继续执行
    else:
        current_app.logger.warning(f"User {current_user.username} attempted to delete ticket ID {ticket_id} without permission")
        ...
        for ticket_id_str in ids:
            try:
                ...
                if current_user.username == 'admin':
                    ...
                    db.session.delete(ticket)
                    deleted_count += 1
                    current_app.logger.info(f"User {current_user.username} deleted own ticket ID {ticket_id}")
                elif hasattr(ticket, 'assignee_name') and ticket.assignee_name and ticket.assignee_name == current_user.username:
                    # 用户可以删除自己负责的工单
                    # 先删除相关的流程记录
                    TicketFlow.query.filter_by(ticket_id=ticket.id).delete()
                    db.session.delete(ticket)
                    deleted_count += 1
                    current_app.logger.info(f"User {current_user.username} deleted ticket ID {ticket_id} as assignee")
                else:
                    current_app.logger.warning(f"User {current_user.username} attempted to delete ticket ID {ticket_id} without permission")
            except ValueError:
                current_app.logger.warning(f"Invalid ticket ID in batch delete: {ticket_id_str}")
...
def export_tickets():
    try:
        ...
        problem_classification_main = str_escape(request.args.get("problem_classification_main", ""))
        ...
        if product_type_level1:
            query = query.filter(Ticket.product_type_level1.like(f"%{product_type_level1}%"))
        if product_type_level2:
            query = query.filter(Ticket.product_type_level2.like(f"%{product_type_level2}%"))
        if problem_classification_main:
            query = query.filter(Ticket.problem_classification_main.like(f"%{problem_classification_main}%"))
        if customer_agent_name:
            query = query.filter(Ticket.customer_agent_name.like(f"%{customer_agent_name}%"))
        if description:
        ...
        writer.writerow([
            'ID', '标题', '描述', '优先级', '状态', '负责人', '照片ID', 
            '图片引用', '影响范围', '处理记录', '解决方案', '服务方式', 
            '预约时间', '工程师ID', '一级产品类型', '二级产品类型', 
            '版本号', '序列号', '是否过保', '下单时间', '完成时间', 
            '主要问题分类', '次要问题分类', '问题分类标签', '问题标签', 
            '创建时间', '更新时间', '客户名称'
        ])
        ...
        for ticket in data:
            writer.writerow([
                ...
                ticket.status, ticket.assignee_name, ticket.photo_ids,
                ticket.image_references_str, ticket.impact_scope, ticket.relatedinfo,
                ticket.solution, ticket.service_method, ticket.appointment_time,
                ticket.engineer_id, ticket.product_type_level1, ticket.product_type_level2,
                ticket.version_number, ticket.serial_number, ticket.is_out_of_warranty,
                ticket.order_time, ticket.completion_time, ticket.problem_classification_main,
                ticket.problem_classification_sub, ticket.problem_classification_tags,
