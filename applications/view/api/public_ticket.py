...
def _allow_request(ip: str, scope: str, limit: int, window_seconds: int):
    now = time.time()
    key = f"{scope}:{ip}"
...
def get_captcha():
    ip = _get_client_ip()
    if not _allow_request(ip, "public_captcha", limit=30, window_seconds=60):
...
def submit_ticket():
    ...
    try:
        ...
        # 检查提交频率
        if not _allow_request(ip, "public_submit", limit=5, window_seconds=60):
            return jsonify({
                'code': 429,
        ...
        # 获取表单数据
        company = str_escape(request.form.get('company', '').strip())
        contact = str_escape(request.form.get('contact', '').strip())
        phone = request.form.get('phone', '').strip()
        problem_type = str_escape(request.form.get('problemType', '').strip())
        description = str_escape(request.form.get('description', '').strip())
            ...
            for image in images:
                if image and image.filename:
                    ...
                    try:
                        ...
                        current_app.logger.warning(f"图片上传失败: {str(upload_error)}")
                        continue
        ...
        db.session.rollback()
        current_app.logger.error(f"工单提交失败: {str(e)}")
        return jsonify({
            'code': 500,
            'msg': '提交失败，请稍后重试'
        })
...
def query_tickets():
    ...
    try:
        ip = _get_client_ip()
        if not _allow_request(ip, "public_query", limit=20, window_seconds=60):
            return jsonify({"code": 429, "msg": "请求过于频繁，请稍后再试"}), 429
        # 获取查询参数
        phone = request.form.get('phone', '').strip()
        captcha = request.form.get('captcha', '').strip()
        ...
        current_app.logger.error(f"工单查询失败: {str(e)}")
        return jsonify({
            'code': 500,
            'msg': '查询失败，请稍后重试'
        })
